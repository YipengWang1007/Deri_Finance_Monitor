...
#other files defined for this blockchain project
...
class MonitorState(ABC):
    #remembers what block we have scanned for later back tracking
    #all class methods here are abstrat methods
    @abstractmethod
    def get_last_scanned_block(self)->int:
        ...
        """
    @abstractmethod
    def start_chunk(self,block_nbumber:int):
...
class Monitor:
    def __init__(self, web3: Web3, contract: Contract, state: MonitorState, events: List, filters: {},
                 max_chunk_scan_size: int = 999, max_request_retries: int = 30, request_retry_seconds: float = 3.0):
        """
    ...
    def get_scan_start_block(self):
        end_block = self.state.get_last_scanned_block()
        if end_block:
            return max(1,end_block-self.NUM_BLOCKS_RESCAN_FOR_FORKS)
        return 1

    def get_scan_end_block(self):
...
class JSONifiedState(MonitorState):
    ...
    def __init__(self):
        self.state = None
        self.fname = "remove_margin_history.json"
    ...
    def process_event(self, block_when: datetime.datetime, event: AttributeDict) -> str:
        ...
        transfer = {
            "owner": args.owner,
            "bTokenId": args.bTokenId,
            "bAmount": args.bAmount,
...
def run():
    ...
    web3 = getWeb3('bsc')
    address = '0x19c2655A0e1639B189FB0CF06e02DC0254419D92'
    contract = web3.eth.contract(address=address, abi=abi)

    #print(contract.all_functions())

    #print(contract.functions.getAddresses().call())
    #print(contract.functions.getStateValues().call())
    #print(contract.functions.getParameters().call())
    ...
    # chain_id: int, web3: Web3, abi: dict, state: EventScannerState, events: List, filters: {}, max_chunk_scan_size: int=4999
    scanner = Monitor(
        web3=web3,
        contract=ERC20,
        state=state,
        events=[ERC20.events.RemoveMargin],
        filters={"address": address},
        # How many maximum blocks at the time we request from JSON-RPC
        # and we are unlikely to exceed the response size limit of the JSON-RPC server
        max_chunk_scan_size=999
    )
    ...
    start_block = max(state.get_last_scanned_block() - chain_reorg_safety_blocks, 7990000)
    end_block = scanner.get_scan_end_block()
